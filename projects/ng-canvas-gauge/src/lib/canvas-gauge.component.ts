 ...
 */
 ...
 private drawGauge() {
   ...
   this.draw270DegreeCircle(this.radius, this.lineWidth);
   this.draw270DegreeCircle(this.radius - (this.lineWidth + 3), 1);
 ...
 ngOnChanges(changes: SimpleChanges): void {
   ...
   if (changes['value'] || changes['max'] ||  changes['radius'] ||  changes['width'] || changes['height'] || changes['ticks']) {
 ...
 private drawArc() {
   ...
   const endAngle = ((this.value / this.max) * 270 + mainStartAngle) * Math.PI / 180;
   ...
   const circleRadius = this.lineWidth  - 2 ; // Adjust the radius of the circle
   const endCircleX = this.center.x + this.radius * Math.cos(endAngle);
   const endCircleY = this.center.y + this.radius * Math.sin(endAngle);
 ...
 private drawNeedle() {
   const startAngle = (-225 * Math.PI) / 180; // Start angle in radians
   const needleLength = this.radius * (2/3); // Adjust the length of the needle as needed
   const needleWidth = this.radius * 1/7; // Adjust the width of the needle end
 ...
 private drawTicks() {
   const centerX = this.gaugeCanvas.nativeElement.width / 2;
   const centerY = this.gaugeCanvas.nativeElement.height / 2;
   const radius = this.radius - (this.lineWidth) - 10; // Adjust this value to control the distance of ticks from the circle center
 ...
 private drawCenterCircle() {
   ...
   const circleRadius = this.radius * (1/7); // Adjust the radius of the circle as needed
 ...
 private drawCenterCircle2() {
   ...
   const circleRadius =  (this.radius * (1/7)) -3 ; // Adjust the radius of the circle as needed
